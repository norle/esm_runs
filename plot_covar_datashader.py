...
def plot_dms_datashader(dm1, dm2, dm1_name='dm1', dm2_name='dm2', gene_name='gene', ax=None, fig=None):
    ...
    min_val_x = 0  # Fixed minimum for ESM distance
    max_val_x = 0.7  # Fixed maximum for ESM distance
    min_val_y = 0  # Fixed minimum for evolutionary distance
    max_val_y = 2.5  # Fixed maximum for evolutionary distance
    
    # Filter points exceeding the axis limits
    df = df[(df['y'] <= max_val_y) & (df['x'] <= max_val_x)]
    
    # Calculate aspect ratio based on fixed ranges
    y_range = max_val_y - min_val_y
    x_range = max_val_x - min_val_x
    aspect_ratio = x_range / y_range
    
    # Create main subplot for datashader plot with fixed ranges
    ...
    # Increase font sizes
    ax.set_xlabel(f'ESM-C distance', fontsize=28)
    ax.set_ylabel(f'Evolutionary distance', fontsize=28)
    ax.set_title(gene_name.upper(), fontsize=32)
...
def process_gene(gene):
    ...
    dm2_name = f'{gene} evolutionary distances'
...
if __name__ == '__main__':
    ...
    # Create a large figure with 3x3 subplots and space for colorbar
    fig = plt.figure(figsize=(24, 24))  # Adjusted for 3x3 layout
    gs = plt.GridSpec(3, 4, width_ratios=[1, 1, 1, 0.05], height_ratios=[1, 1, 1])
    ...
    for idx, (dm1, dm2, dm1_name, dm2_name, gene) in enumerate(all_results):
        row = idx // 3  # Changed to 3 for new layout
        col = idx % 3   # Changed to 3 for new layout
    ...
    cbar.ax.tick_params(labelsize=24)  # Increased colorbar tick font size
    cbar.set_label('Density', size=28)  # Increased colorbar label font size
